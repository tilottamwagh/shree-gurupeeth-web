...
export default function Home(){
 ...
 return <main className="shell">
  ...
  {view==="home"&&<><section className="hero"><div><em>॥ श्री स्वामी समर्थ ॥</em><h1>सेवा, साधना आणि<br/>संस्कारांचा मार्ग</h1><p>नित्यसेवा, आरती, मंत्रजप आणि सेवा मार्गाची माहिती आता एका ठिकाणी.</p><button className="primary" onClick={()=>setView("nitya")}>आजची नित्यसेवा →</button></div><div className="portrait"><i/><Image src="/app-assets/swami_samarth.png" alt="श्री स्वामी समर्थ" width={340} height={340} priority/></div></section><section className="content"><div className="heading"><div><em>आपली दैनिक सेवा</em><h2>सेवा सुविधा</h2></div></div><div className="grid">{services.map(x=><button className="card" key={x.id} onClick={()=>openService(x.id)}><span><Image src={`/app-assets/${x.img}`} alt="" width={52} height={52}/></span><b>{x.title}</b><small>{x.sub}</small></button>)}</div></section><section className="quote">“<p>भिऊ नकोस, मी तुझ्या पाठीशी आहे.</p><small>— श्री स्वामी समर्थ</small></section><section className="content"><div className="heading"><div><em>सेवा कार्य</em><h2>आपली केंद्रे</h2></div></div><div className="centers">{centers.map(([t,img])=><article key={t}><Image src={`/app-assets/${img}`} alt={t} fill/><div><b>{t}</b><small>माहिती पहा →</small></div></article>)}</div></section></>}
  {(view==="nitya"||view==="charitra")&&<section className="page reader-page"><button className="back" onClick={()=>selected?setSelected(null):setView("home")}>← {selected?"सूची":"मुख्यपृष्ठ"}</button><em>{view==="nitya"?"दैनिक उपासना":"श्री स्वामी चरित्र सारामृत"}</em><h2>{selected?.title||(view==="nitya"?"नित्यसेवा स्तोत्र व मंत्र":"स्वामी चरित्र")}</h2>{selected?<article className="devotional-text">{selected.text}</article>:loading?<div className="loading">अधिकृत मजकूर उघडत आहे…</div>:<div className="list">{entries.map((x,i)=><button key={x.title} onClick={()=>setSelected(x)}><span>{String(i+1).padStart(2,"0")}</span><b>{x.title}</b><strong>›</strong></button>)}</div>}</section>}
  {info&&<section className="page"><button className="back" onClick={()=>setView("home")}>← मुख्यपृष्ठ</button><em>श्री गुरुपीठ</em><h2>{info.title}</h2><p>{info.intro}</p><div className="list">{info.items.map((x,i)=><button key={x}><span>{String(i+1).padStart(2,"0")}</span><b>{x}</b><strong>›</strong></button>)}</div><p className="source-note">या विभागाचा सविस्तर मजकूर मूळ ॲपमध्ये ऑनलाइन सर्व्हरवरून येतो. APK मध्ये उपलब्ध नसलेली माहिती अंदाजाने दाखवलेली नाही.</p></section>}
  ...
  {view==="about"&&<section className="page"><button className="back" onClick={()=>setView("home")}>← मुख्यपृष्ठ</button><em>सेवा मार्ग</em><h2>श्री गुरुपीठ विषयी</h2><Image className="about" src="/app-assets/card_dindoripranit.png" alt="सेवा मार्ग" width={700} height={250}/><p>दिंडोरी प्रणीत श्री स्वामी समर्थ सेवा मार्गाचे हे अधिकृत डिजिटल व्यासपीठ आहे. सेवेकरी आणि भाविकांना नित्यसेवा व सेवा कार्याशी जोडणे हा याचा उद्देश आहे.</p></section>}
